...
def download_pics(*taxa, **kwargs):
    ...
    for taxon in taxa:
        ...
        print('Downloading pic for %s...' % taxon, end=' ')
        sys.stdout.flush()
...
def image_url(taxon, max_images=1, img_format='svg'):
    ...
    thumbnails = sorted(list(thumbnail_counts.items()),
...
def pic_search(taxon):
    url = name_search_url % urllib.parse.quote_plus(taxon)
...
if __name__ == '__main__':
    if len(sys.argv) > 1: taxa = sys.argv[1:]
        ...
        taxa = input('Enter names of taxa, separated by commas: ')
